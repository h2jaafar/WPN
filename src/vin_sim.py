...
from resources.datasets.vin.dataset import *
from algorithms.vin.utils import *
from algorithms.vin.vin import *

from algorithms.configuration.maps.gridworld import *
from algorithms.vin.obstacle_gen import *
...
def main(config,
    ...
    for dom in range(n_domains):
        ...
        # Uses obstacles class, pass the (imsize,imsize), (goalx,goaly) and max obs size
        obs = obstacles([config.imsize, config.imsize], goal, max_obs_size)
        # Add obstacles to map, use add_n_rand_obx method from obstacle class
        n_obs = obs.add_n_rand_obs(max_obs) 
        # Add border to map, use add_border method
        border_res = obs.add_border()
        # Ensure we have valid map
        if n_obs == 0 or not border_res:
            continue
        # Get final map
        im = obs.get_final() #Use method from obstacles class to put it all together 
